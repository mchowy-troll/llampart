<script lang="ts">
	...
	async function executePrompt(prompt: MCPPromptInfo, args: Record<string, string>) {
		...
		try {
			...
			const errorMessage =
				error instanceof Error ? error.message : t('mcp.unknownErrorExecutingPrompt');
...
>
				...
				{#snippet titleExtra()}
					{#if prompt.arguments?.length}
						<Badge variant="secondary">
							...
							{prompt.arguments.length > 1 ? ` ${t('mcp.args')}` : ` ${t('mcp.arg')}`}
						</Badge>
					{/if}
				{/snippet}
			</ChatFormPickerItemHeader>
	...
	{:else}
		<ChatFormPickerList
			...
			{showSearchInput}
			searchPlaceholder={t('mcp.searchPrompts')}
			emptyMessage={t('mcp.noPromptsAvailable')}

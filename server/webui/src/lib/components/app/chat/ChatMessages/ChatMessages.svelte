<script lang="ts">
	...
	let displayMessages = $derived.by(() => {
		...
		const showSystemMessage = Boolean(currentConfig.showSystemMessage ?? true);
		const filteredMessages = showSystemMessage
			? messages
			: messages.filter(
					(msg) => msg.role !== MessageRole.SYSTEM && msg.type !== MessageRole.SYSTEM
				);

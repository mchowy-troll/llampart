<script lang="ts">
	...
	function getCountLabel(
		...
		if (polishMany && singular === 'zasób' && plural === 'zasoby') {
			if (count === 1) return singular;
			const mod10 = count % 10;
			const mod100 = count % 100;
			if (mod10 >= 2 && mod10 <= 4 && !(mod100 >= 12 && mod100 <= 14)) return plural;
			return polishMany;
		}

		if (polishMany && singular === 'szablon' && plural === 'szablony') {
			if (count === 1) return singular;
			const mod10 = count % 10;
			const mod100 = count % 100;
			if (mod10 >= 2 && mod10 <= 4 && !(mod100 >= 12 && mod100 <= 14)) return plural;
...
<Collapsible.Root open={isExpanded} onOpenChange={onToggleServer}>
	...
	>
		...
		<span class="inline-flex flex-col items-start text-left">
			...
			<span class="text-xs text-muted-foreground">
				...
				{getCountLabel(
					serverRes.resources.length,
					t('mcp.resourcesCountLabelSingular'),
					t('mcp.resourcesCountLabelPlural'),
					'zasobów'
				)}{#if hasTemplates}, {serverRes.templates.length}
					{getCountLabel(
						serverRes.templates.length,
						t('mcp.templatesCountLabelSingular'),
						t('mcp.templatesCountLabelPlural'),
						'szablonów'
					)}{/if})
			</span>
		</span>
	...
	<Collapsible.Content>
		<div class="ml-4 flex flex-col gap-0.5 border-l border-border/50 pl-2">
			...
			{:else if !hasContent}
				<div class="py-1 text-xs text-muted-foreground">{t('mcp.noResources')}</div>
			{:else}
				{#if hasResources}

<script lang="ts">
	...
	async function loadContent(uri: string) {
		...
		try {
			...
			if (result) {
				...
				error = t('mcp.failedToLoadResourceContent');
			}
		} catch (e) {
			error = e instanceof Error ? e.message : t('mcp.unknownError');
		} finally {
			isLoading = false;
		}
	}
	...
	function handleDownload() {
		...
		downloadResourceContent(
			...
			resource.name || t('mcp.resourceFileFallback')
		);
	}
</script>
...
<div class={cn('flex flex-col gap-3', className)}>
	{#if !resource}
		<div class="flex flex-col items-center justify-center gap-2 py-8 text-muted-foreground">
			...
			<span class="text-sm">{t('mcp.selectResourceToPreview')}</span>
		</div>
	{:else}
				...
				{#each blobContent as blob (blob.uri)}
					...
					{:else}
						<div class="flex items-center gap-2 rounded bg-muted p-2 text-sm text-muted-foreground">
							...
							<span>{t('mcp.binaryContent')} ({blob.mimeType || t('mcp.unknownType')})</span>
						</div>
					{/if}
				{/each}
			...
			<div class="flex flex-wrap gap-2 text-xs text-muted-foreground">
				...
				{#if resource.annotations?.priority !== undefined}
					<span class="rounded bg-muted px-1.5 py-0.5">
						{t('mcp.priority')}: {resource.annotations.priority}
					</span>
				{/if}

				<span class="rounded bg-muted px-1.5 py-0.5">
					{t('mcp.server')}: {resource.serverName}
				</span>
			</div>
		{/if}
	{/if}

<script lang="ts">
	...
	let newServerUrlError = $derived.by(() => {
		if (!newServerUrl.trim()) return t('mcp.urlRequired');
		try {
...
<div class="space-y-4">
	<div class="flex items-center justify-between gap-4 border-b border-border/30 pb-3">
		<div>
			<h4 class="text-base font-semibold">{t('mcp.manageServers')}</h4>
		</div>
	...
	{#if isAddingServer}
		<Card.Root class="bg-muted/30 p-4">
			<div class="space-y-4">
				<p class="font-medium">{t('mcp.addNewServer')}</p>

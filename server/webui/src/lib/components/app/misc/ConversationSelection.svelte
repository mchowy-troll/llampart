<script lang="ts">
	...
	let filteredConversations = $derived(
		conversations.filter((conv) => {
			const name = conv.name || t('common.untitledConversation');
	...
	let selectedStatusText = $derived.by(() =>
		(selectedIds.size === 1
			? t('settings.importExport.selectedCountOne')
			: t('settings.importExport.selectedCountMany')
		)
			.replace('{selected}', String(selectedIds.size))
			.replace('{total}', String(conversations.length))
	);

	let shownStatusText = $derived.by(() =>
		(filteredConversations.length === 1
			? t('settings.importExport.shownCountOne')
			: t('settings.importExport.shownCountMany')
		).replace('{count}', String(filteredConversations.length))
	);
...
<div class="space-y-4">
			...
			<table class="w-full">
				<thead class="sticky top-0 z-10 bg-muted">
					<tr class="border-b">
						...
						<th class="p-3 text-left text-sm font-medium">{t('common.conversationName')}</th>

						<th class="w-32 p-3 text-left text-sm font-medium">{t('common.messages')}</th>
					</tr>
				</thead>
				<tbody>
							...
							>
								...
								<td class="p-3 text-sm">
									<div
										...
										title={conv.name || t('common.untitledConversation')}
									>
										{conv.name || t('common.untitledConversation')}
									</div>
								</td>
	...
	<div class="flex justify-end gap-2">
		...
		<Button onclick={handleConfirm} disabled={selectedIds.size === 0}>
			{mode === 'export'
				? t('settings.importExport.exportSelectedButton')
				: t('settings.importExport.importSelectedButton')}
			({selectedIds.size})
		</Button>
	</div>
</div>

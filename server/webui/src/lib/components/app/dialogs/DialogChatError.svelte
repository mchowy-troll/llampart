<script lang="ts">
	...
	const title = $derived(isTimeout ? t('dialogs.tcpTimeoutTitle') : t('dialogs.serverErrorTitle'));
	const description = $derived(
		isTimeout ? t('dialogs.tcpTimeoutDescription') : t('dialogs.serverErrorDescription')
	);
	...
	<AlertDialog.Content>
		...
		<div class={`rounded-lg border px-4 py-3 text-sm ${badgeClass}`}>
			...
			{#if contextInfo}
				<div class="mt-2 space-y-1 text-xs opacity-80">
					<p>
						<span class="font-medium">{t('dialogs.promptTokensLabel')}</span>
						{contextInfo.n_prompt_tokens.toLocaleString()}
					</p>
					{#if contextInfo.n_ctx}
						<p>
							<span class="font-medium">{t('dialogs.contextSizeLabelInline')}</span>
		...
		<AlertDialog.Footer>
			<AlertDialog.Action onclick={() => handleOpenChange(false)}
				>{t('common.close')}</AlertDialog.Action
			>
		</AlertDialog.Footer>
	</AlertDialog.Content>
</AlertDialog.Root>

	...
	<AlertDialog.Content class="max-w-lg">
		<AlertDialog.Header>
			<AlertDialog.Title class="flex items-center gap-2">
				...
				{t('dialogs.modelNotAvailableTitle')}
			</AlertDialog.Title>

			<AlertDialog.Description>
				{t('dialogs.modelNotAvailableDescription')}
			</AlertDialog.Description>
		</AlertDialog.Header>
		...
		<div class="space-y-3">
			...
			{#if availableModels.length > 0}
				<div class="text-sm">
					<p class="mb-2 font-medium text-muted-foreground">{t('dialogs.selectAvailableModel')}</p>
		...
		<AlertDialog.Footer>
			<AlertDialog.Action onclick={() => handleOpenChange(false)}
				>{t('common.cancel')}</AlertDialog.Action
			>
		</AlertDialog.Footer>
	</AlertDialog.Content>
</AlertDialog.Root>

...
Current desktop and chat layout scale primitives are defined in `src/lib/styles/tokens.css` and consumed by `llampart-desktop-fluid-ui-scale` plus `llampart-chat-layout-scale-primitives` in `src/app.css`. Components keep semantic structure, while root UI scale, chat/composer widths and shared scroll/message spacing are tunable from the layout token owner. The desktop auto-scale keeps the 1080p baseline at 12px and caps large desktop viewports at 14.4px, matching the accepted 90% target for 2560 CSS px and above. The optional user UI scale preset is applied as a separate 90% / 100% / 110% multiplier in the same desktop scale owner layer, so the accepted responsive base remains independently understandable.
